...
%An enormous growth of structured data is being made available on the Web as Open or Linked Data\todo{CL: I wrapped this into \textbackslash url; please do the same everywhere.}\footnote{\url{http://lod-cloud.net/}} as well as Big Data within organisations. 
...
Despite the growing interest and use in both research and the industry communities, current 
\todo{*CL: ``data management solutions benchmark creators''?! -- Rephrase to give it a more comprehensible structure!  Do you mean ``creators of benchmarks for data management solutions''? Jeremy: is benchmarking tools better? I have no idea how to make this better}\todo{CL: Are you sure this is a scientific term?  ``Solution'' sounds like marketing jargon.}\todo[color=green]{Harsh: data management software/platform/solution/framework can/are be/being used interchangeably, given the purpose of use.} 
creators of benchmarks for Data Management Solutions (DMS)~\cite{alucc2014diversified,Bizer2009TheBS} do not offer a common 
\todo{CL: the relevance of the cross-domain aspect is not explained well.  What are different domains of interest, what existing DMSs are limited to specific domains, why is a cross-domain analysis of interest?}
suite for performing task-specific benchmarks.
In addition, there is no single baseline to compare these cross-domain DMS one against the other. 
...
Results in areas such as named entity recognition and linking~\cite{gerbil} as well as question answering~\cite{bioasq,qald} have, however, shown that the provision of standardised interfaces and measures \todo{CL: As the following statement is important to our research (because our work is basically based on the same assumption) it wouldn't hurt to add one sentence more detail, i.e.\ in what \emph{way} these contribute to improvements.}can contribute to the improvement of the performance of software solutions.
...
In addition to surveying existing work, we intend to focus mainly on the purpose and scope of the benchmarks.
    ...
    The aim of the Lehigh University Benchmark (\textbf{LUBM})~\cite{Guo:2005:LBO:1741305.1741322} is to evaluate the performance of Semantic Web \todo[color=green]{CL: If ``repository'' means ``triple store'' then use the word ``triple store'' for consistency with the rest of this paper.}repositories over a large synthetic dataset that complies to a university domain ontology.
...
\section{Objectives, Challenges and Outcomes}\label{Objectives}
    ...
    \subsection{Challenges to be addressed}\label{challenges}
        ...
        \begin{itemize}[nosep]
            ...
            %Complex performance indicators should be explored 
            %To conduct a correlation analysis of the impacts of diverse types of queries and data on the storage and indexing efficacy of DMSs, it is necessary to explore more complex performance indicators than the traditional ones of
        \end{itemize}
