...
\section{Introduction}\label{sec:Introduction}
    ...
    Despite they are widely used on research community and the growing interest of industry, current benchmarks (individually or as a combined systems) do not offer one widespread suite of benchmarks and no single unit of verifications of the results for a whole systems. In order to fill the gaps (i) to help end user to choose the right system by showing the ranked results of the system's behaviors for the different parameter selection permutation, and (ii) the involvement of industry community to improve their systems by modeling a different challenges and pick a right pipeline for their needs. 
    
    In this paper we present LITMUS, an open extensible solution towards benchmarking a wide variety of NoSQL data management solutions (DMS). LITMUS is focused to to support organizations/institutions which aspire to use Linked Data management technologies in a wide spectrum of applications and magnitudes. In doing so, LITMUS will provide realistic performance evaluation platform covering a plethora of heterogeneous technologies (see \ref{litmus_framework}) for storage and querying benchmarking. The major objectives of LITMUS are:
     \begin{itemize}
